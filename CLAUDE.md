...
A custom Minecraft client built from scratch, inspired by Lunar Client.

## Project Vision

A performance-focused, feature-rich Minecraft client with:
- Custom launcher/UI
- FPS optimizations
- Quality-of-life mods (minimap, keystrokes, FPS counter, CPS, armor HUD, etc.)
- Cosmetics system
- Custom capes/emotes
- Server list integration
- Multi-version support (1.8.9 priority since it's the PvP standard, plus modern versions)

## Tech Stack (locked 2026-04-20)

**Target version:** Minecraft **1.8.9 only**.

**Launcher:** Electron + TypeScript + React
- Same stack Lunar Client uses (confirmed — Lunar's launcher process is `Lunar Client.exe`, an Electron 25+ app).
- Launcher UI perf is irrelevant to in-game FPS; the Minecraft jar runs as a separate JVM process.
- Packaged with `electron-builder`.

**Game client:** Forge 1.8.9 mod + Java 8
- Forge is the de facto standard for 1.8.9 — nearly all 1.8.9 mods and mappings target it.
- Build: Gradle + ForgeGradle + MCP mappings (SRG/stable).
- Bytecode manipulation via Mixin for patches Forge events can't cover.

**Auth:** Microsoft OAuth (MSAL, device code flow). Mojang auth is dead — MS accounts only.

**Launch flow:** Electron launcher → downloads/verifies 1.8.9 vanilla jar → injects our Forge mod → spawns JVM with correct classpath + args → monitors process for crash reports.
...
wiiwho-client/
├── CLAUDE.md              # this file
├── launcher/              # desktop launcher app (Electron or Tauri)
├── client-mod/            # Fabric/Forge mod with features
├── assets/                # icons, splash, cosmetics
...
These can be invoked with `Skill` or typed as `/skill-name`:

- **`/gsd:new-project`** — Initialize project with deep context gathering. Best first move for this project.
- **`/gsd:new-milestone`** — Break work into milestones (v0.1 launcher, v0.2 mod base, etc.).
- **`/gsd:plan-phase`** — Detailed plan for a single phase (e.g., "build Microsoft auth flow").
- **`/gsd:execute-phase`** — Actually implement a planned phase.
- **`/gsd:research-phase`** — Research unfamiliar territory (e.g., how Fabric mixins work) before planning.
- **`/gsd:ui-phase`** — Generate UI design contract — useful for the launcher UI.
- **`/gsd:debug`** — Systematic debugging across context resets — invaluable for tricky mod crashes.
- **`/gsd:map-codebase`** — Once we've vendored Minecraft/Fabric sources, map them.
- **`/init`** — Upgrade this CLAUDE.md once repo has real code.
- **`/review`** — Pre-merge review on PRs.
- **`/simplify`** — Quality pass on changed code.
- **`/fewer-permission-prompts`** — Reduce friction once we settle on build commands.

## Useful MCP Servers

Configured on this machine:

- **Context7** (`mcp__claude_ai_Context7__*`) — Fetch current docs for any library/framework. Critical for:
  - Fabric API (changes fast)
  - Electron / Tauri
  - React / any launcher UI framework
  - Gradle / Loom
  - MSAL (Microsoft auth)
- **Excalidraw** (`mcp__claude_ai_Excalidraw__*`) — Diagram architecture (auth flow, mod→launcher IPC, rendering pipeline).
- **Gmail / Google Calendar / Drive** — Not directly relevant to client dev; skip unless coordinating release comms.

## External References We'll Need

(Populate as we go — Context7 can resolve library IDs on demand)

- Fabric docs — `fabricmc.net/wiki`
- Fabric API source — `github.com/FabricMC/fabric`
- Yarn mappings — `github.com/FabricMC/yarn`
- Minecraft protocol — `wiki.vg`
- Microsoft auth for Minecraft — `wiki.vg/Microsoft_Authentication_Scheme`
- Mixin (bytecode manipulation framework) — `github.com/SpongePowered/Mixin`

## Non-Goals (explicit)

- No cheats, ghost clients, or PvP unfair-advantage mods. This is a QoL client.
- No cracked-account support. Microsoft auth only.
- No re-distribution of Minecraft assets — launcher downloads official jars.

## Legal Notes

- Minecraft EULA allows mods but prohibits monetizing mods tied to Mojang assets. If we ever charge, it's for cosmetics/features in *our* code, never Mojang's.
- Microsoft/Mojang can revoke API access. Budget for that risk.

## Open Questions

- [ ] Cosmetics served from our backend or client-side only at v0.1?
- [ ] Licensing — MIT, GPL, or proprietary?
- [ ] Which features from the Lunar-style mod list are in scope for v0.1?

## Current Status

**2026-04-20** — Repo initialized. Nothing built yet. Next step: decide tech stack + scope v0.1.
